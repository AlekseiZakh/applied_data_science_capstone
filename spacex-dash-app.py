...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # If ALL sites are selected, show total success launches for all sites
        fig = px.pie(
            spacex_df, 
            values='class',
            names='Launch Site',
        ...
        site_df['class'] = site_df['class'].map({1: 'Success', 0: 'Failure'})
...
def get_scatter_chart(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        # If ALL sites are selected, show all data within the payload range
        fig = px.scatter(
            filtered_df,
            x='Payload Mass (kg)',
            y='class',
...
# Run the app
if __name__ == '__main__':  # Note: Double underscores, not double asterisks
    app.run()